...
public:
    ...
    ~DelegatePImpl() {
        if (FUNC == kind) functional.~FunctionType();
        else if (FPA == kind) obj.~A();
    }
    ...
    static inline R IRAM_ATTR vPtrToFunAPtrExec(void *self, P... args) ALWAYS_INLINE_ATTR {
        return static_cast<DelegatePImpl *>(self)->fnA(
            static_cast<DelegatePImpl *>(self)->obj, std::forward<P...>(args...)
        );
    };
...
public:
    DelegatePImpl() { fn = nullptr; }

    DelegatePImpl(std::nullptr_t) { fn = nullptr; }

    DelegatePImpl(const DelegatePImpl &del) { fn = del.fn; }

    DelegatePImpl(DelegatePImpl &&del) { fn = std::move(del.fn); }

    DelegatePImpl(FunPtr fn) { DelegatePImpl::fn = fn; }

    template <typename F> DelegatePImpl(F fn) { DelegatePImpl::fn = std::forward<F>(fn); }
    ...
    inline IRAM_ATTR operator bool() const ALWAYS_INLINE_ATTR { return fn; }

    operator FunVPPtr() const { return vPtrToFunPtrExec<R, P...>; }

    void *arg() const { return reinterpret_cast<void *>(fn); }
...
public:
    DelegateImpl() { fn = nullptr; }

    DelegateImpl(std::nullptr_t) { fn = nullptr; }

    DelegateImpl(const DelegateImpl &del) { fn = del.fn; }

    DelegateImpl(DelegateImpl &&del) { fn = std::move(del.fn); }

    DelegateImpl(FunPtr fn) { DelegateImpl::fn = fn; }

    template <typename F> DelegateImpl(F fn) { DelegateImpl::fn = std::forward<F>(fn); }
    ...
    operator FunVPPtr() const { return reinterpret_cast<FunVPPtr>(fn); }

    void *arg() const { return nullptr; }
...
public:
    ...
    Delegate(F functional)
        : detail::DelegatePImpl<A *, R, P...>::DelegatePImpl(std::forward<F>(functional)) {}
...
public:
    ...
    template <typename F>
    Delegate(F functional)
        : detail::DelegatePImpl<void, R, P...>::DelegatePImpl(std::forward<F>(functional)) {}
...
public:
    ...
    Delegate(Delegate &&del)
        : detail::DelegateImpl<A, R>::DelegateImpl(
...
public:
    ...
    Delegate(const Delegate &del)
        : detail::DelegateImpl<void, R>::DelegateImpl(
...
public:
    ...
    template <typename F>
    Delegate(F functional)
        : delegate::detail::Delegate<void, R, P...>::Delegate(std::forward<F>(functional)) {}
