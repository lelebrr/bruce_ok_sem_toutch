...
public:
     ...
     */
    template <typename T> T getManufacturerData(bool skipSizeCheck = false) {
     ...
     */
    template <typename T> T getServiceData(uint8_t index = 0, bool skipSizeCheck = false) {
     ...
     * @details <b>Use:</b> <tt>getServiceData<type>(skipSizeCheck);</tt>
     */
    template <typename T> T getServiceData(const NimBLEUUID &uuid, bool skipSizeCheck = false) {
...
public:
     ...
     *
     * As we are scanning, we will find new devices.  When found, this call back is invoked with a reference
     * to the device that was found.  During any individual scan, a device will only be detected one time.
     */
