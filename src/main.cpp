...
void setup() {
    ...
    log_d("Total heap: %d", heap_caps_get_total_size(MALLOC_CAP_DEFAULT));
    log_d("Free heap: %d", heap_caps_get_free_size(MALLOC_CAP_DEFAULT));
    if (esp_psram_is_initialized()) log_d("PSRAM Started");
    log_d("Total PSRAM: %d", heap_caps_get_total_size(MALLOC_CAP_SPIRAM));
