...
void runClockLoop() {
    ...
    while (true) {
        if (clock_set) {
            ...
            tft.drawCentreString(timeBuf, tftWidth / 2, tftHeight / 2, 4);
        } else {
            tft.drawCentreString("Clock not set", tftWidth / 2, tftHeight / 2, 4);
        }
        delay(1000);
...
// Stubs for missing menu functions
void addEvilWifiMenu() {}
void removeEvilWifiMenu() {}

// Stub functions for mykeyboard
uint32_t getComplementaryColor2(uint32_t color) { return color; } // stub, return same
