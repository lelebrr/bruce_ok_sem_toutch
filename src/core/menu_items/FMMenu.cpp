...
void FMMenu::drawIconImg() {
    if (bruceConfig.theme.fs) {
        drawImg((fs::FS &)SD, bruceConfig.getThemeItemImg(bruceConfig.theme.paths.fm), 0, imgCenterY, true);
    } else {
        drawImg(LittleFS, bruceConfig.getThemeItemImg(bruceConfig.theme.paths.fm), 0, imgCenterY, true);
    }
}
