...
void WifiMenu::optionsMenu() {
    ...
    if (!wifiConnected) {
        options.clear();
        options.push_back({"Connect Wifi", lambdaHelper(wifiConnectMenu, WIFI_STA)});
        options.push_back({"WiFi AP", [=]() {
                               wifiConnectMenu(WIFI_AP);
                               displayInfo("pwd: " + bruceConfig.wifiAp.pwd);
                           }});
    } else {
        options.clear();
        options.push_back({"Disconnect", wifiDisconnect});
...
void WifiMenu::configMenu() {
    options.clear();
    options.push_back({"Add Evil Wifi", addEvilWifiMenu});
    options.push_back({"Remove Evil Wifi", removeEvilWifiMenu});
