...
void FileMenu::drawIconImg() {
    if (bruceConfig.theme.fs) {
        drawImg(
            (fs::FS &)SD, bruceConfig.getThemeItemImg(bruceConfig.theme.paths.files), 0, imgCenterY, true
        );
    } else {
        drawImg(LittleFS, bruceConfig.getThemeItemImg(bruceConfig.theme.paths.files), 0, imgCenterY, true);
    }
}
