...
void rfTxCallback(cmd *c) {
    ...
    if (!initRfModule("tx", float(frequency / 1000000.0))) return;

    RCSwitch_send(key, bits, te, 1, count);
    deinitRfModule();
}
void rfScanCallback(cmd *c) {
    // subghz scan 433 434
...
void createRfCommands(SimpleCLI *cli) {
    Command cmd = cli->addCmd("rf,subghz");
